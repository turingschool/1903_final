...
class CuratorTest < Minitest::Test
  ...
  def test_photographs_value_is_equal_to
    assert_equal [], @curator.photographs
  end

  def test_photos_can_be_add_to_photographs_array
    @curator.add_photograph(@photo_1)
    @curator.add_photograph(@photo_2)
    assert_equal [@photo_1, @photo_2], @curator.photographs
  end

  def test_artists_value_is_equal_to
    assert_equal [], @curator.artists
  end

  def test_artists_can_be_added_to_artists_array
  ...
  def test_curator_can_return_artists_with_multiple_photographs
    skip
    @curator.add_artist(@artist_1)
    @curator.add_artist(@artist_2)
    @curator.add_artist(@artist_3)
  ...
  def test_curator_can_find_taken_by_artists_from_a_specific_country
    @curator.add_artist(@artist_1)
    @curator.add_artist(@artist_2)
    @curator.add_artist(@artist_3)
    @curator.add_photograph(@photo_1)
